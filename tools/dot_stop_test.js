// 敵のDOT停止演出(effects.js playEnemyDotStopSequence)の単体テスト(2026-07-31)。
// シーケンスがsetTimeoutで進む非同期処理のため、他のスモークと違いテスト本体の完了
// (window.__failedへの書き込み)をポーリングで待ってから終了する。
// 実行方法: cd C:\温泉村物語\tools && node dot_stop_test.js
const fs = require("fs");
...
const testBody = fs.readFileSync(path.join(__dirname, "dot_stop_body.js"), "utf8");
...
// 非同期テストの完了(__failedへの書き込み)を待つ。20秒で打ち切り
