...
(async () => {
  ...
  await page.selectOption("#questTestSelect", "wasureyu_oshira");
  ...
  await page.screenshot({ path: "tmp/kaerazu_route.png" });
  ...
  if (pre) {
    await page.screenshot({ path: "tmp/kaerazu_prebattle.png" });
  ...
  const st2 = await page.evaluate(() => ({
    ...
    formAnnounce: battleLogLines.some((l) => l.includes("まずは、ぬるめから")),
    gimmickAnnounce: battleLogLines.some((l) => l.includes("湯気の色が、変わり始める")),
  }));
  console.log("ボス戦:", JSON.stringify(st2));
  await page.waitForTimeout(800);
  await page.screenshot({ path: "tmp/kaerazu_boss.png" });
