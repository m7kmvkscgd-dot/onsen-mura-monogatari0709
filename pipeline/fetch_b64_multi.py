...
def build_b64_json(results_path, out_path):
    ...
    for key, r in results.items():
        ...
        for fname in r["filenames"]:
            img = Image.open(COMFY_OUTPUT / fname).convert("RGB")
            buf = io.BytesIO()
            img.save(buf, format="JPEG", quality=90)
            out[key].append(base64.b64encode(buf.getvalue()).decode("ascii"))
    with open(out_path, "w", encoding="utf-8") as f:
        json.dump(out, f)
    print(f"wrote {out_path}: {sum(len(v) for v in out.values())} images")
