...
// showSoulStoryOffer/openSoulStoryViewer)は休眠状態で残置=敵defにsoulStoryを足せばそのまま復活する。
// 以後のクエストは「ステージ+敵+ボス+テキストの物語(依頼文/区間フレーバー/口上/完了文)」構成、
// クエスト専用背景の新規制作も停止(既存2ルートの背景は残す)。回想素材はassets/story/等に残置

// ============ 重傷=温泉療養システム(2026-08-01、パーマデス標準廃止) ============
// 戦闘不能はロストではなく「重傷」(status:"injured")になり、この日数のあいだ編成不可
// (温泉村での湯治という世界観に接続)。復帰時はHP全快、ただし倒れた時のストレス+100は残る。
// 設定の「パーマデスモード」トグルON(state.permadeathMode)の時だけ従来の即ロストになる。
// 英霊システム(戦死者の墓)構想は2026-08-01廃案
const INJURY_REST_DAYS = 2;

// ============ 戦闘レイアウトプリセット(2026-07-31) ============
// 商業クオリティ化の一環でユーザーがmock_battle_layout_editor.htmlで調整した新配置。
//   "classic"    … 従来レイアウト(v2.3まで: 敵列が画面上端、ログはその下、味方バー284px)。
//                  CSS/JSは一切削除していないので、この定数を"classic"へ変えるだけで完全に元へ戻る
//   "center_v1"  … 2026-07-31ユーザー確定値: ログを上部(10.1vh)へ、敵列を画面中央帯(26vh)へ、
//                  味方バーを49.2vh へ。コマンド列は従来どおり味方バーの実測位置へ自動追従。
//                  適用はrenderBattleScreen(battle.js)が.battle-top/#battlePartyBarへ
//                  .layout-centerクラスを付け外しする(battle.cssの同名セクション参照)
