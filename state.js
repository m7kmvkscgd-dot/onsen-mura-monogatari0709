...
function defaultState() {
  return {
    ...
    magistrateLevel: 0, // 増築の1つ、奉行所のレベル(0=未建築、1=建築済み。村レベル2で解禁)。依頼を受けられるようになる
    ...
    magistrateQuestFailedOn: {}, // { questId: dayCount } クエストダンジョン依頼(モンハン形式)に失敗した日。当日は再受注不可、翌日は必ず再張り出しされる(refreshMagistrateQuestsIfNeeded参照)
    ...
    catapultLevel: 0, // 増築の1つ、投石器のレベル(0=未建築、1=建築済み。村レベル4で解禁)。襲撃戦のラウンド終了ごとに地上の敵1体へ自動で投石攻撃する
...
const ROSTER_CAPACITY = 8; // 仲間を雇える上限。以前は村レベルに連動して3→10人まで伸びる仕組みだったが、
// ユーザー指示により村レベルとは完全に切り離し、初期から固定で8人雇えるようにした(村レベルは施設解禁専用)
...
// 投石器(2026-07-29ユーザー指示): 襲撃戦のラウンド終了ごとに、地上の敵(isFlyingでない)からランダムに
// 1体選んで自動で投石攻撃する。威力は「村レベル=槍士の同レベル攻撃力」として通常攻撃と同じ式で
// 計算する(raid.jsのfireCatapultOnRoundEnd参照)。敵が飛行ユニットだけの時は対象がおらず発動しない
const CATAPULT_UNLOCK_HOUSE_LEVEL = 4;
const CATAPULT_COST = 100;
const CATAPULT_MATS = { ki: 8, kawa: 2 };
