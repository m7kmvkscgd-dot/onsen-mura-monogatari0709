...
// 適用したもの)。里に戻る(finishRetreat)か新しい遠征を始める(enterDungeon)と自動的に見送った扱いになり、
// 次の遠征には持ち越さない(討伐依頼のような複数日にまたがる長期の追跡ではなく、その場の緊張感が目的のため)
...
function enterDungeon() {
  currentFloor = 1;
  retreating = false;
  bossPursuit = null; // 前回の遠征の追撃状態を持ち越さない(新しい遠征は必ず追撃なしから始まる)
...
function finishRetreat() {
  ...
  stopCoastAreaBgm();
  retreating = false;
  bossPursuit = null; // 里に戻った時点で追撃モードは終了(見送った扱い、次の遠征には持ち越さない)
