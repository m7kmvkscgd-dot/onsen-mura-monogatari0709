...
function moveOneFloor(pathBias, enterTeahouse) {
  ...
  if (retreating) {
    ...
    if (pathBias === CAVE_FORK_KEY) {
      ...
      currentFloor = 1;
      pathBias = null;
    } else if (pathBias === CASTLE_FORK_KEY) {
      // 廃城下町の分かれ道で「古城方向(門)」を選んだ: 廃城下町側のこの階層を覚えておき、門1層目から数え直す
      stageEntryStack.push({ stage: currentStage, floor: currentFloor + 1 });
      currentStage = "gate";
      currentFloor = 1;
      pathBias = null;
    } else if (currentFloor >= (STAGE_CHAIN_MAX[currentStage] || Infinity) && STAGE_CHAIN_NEXT[currentStage]) {
      // 中継ステージの最深部からの自動継続(洞窟→廃城下町→少し森→海の村、渓流→光る竹林)。選択の余地の
...
document.getElementById("advanceBtn").onclick = () => {
  ...
  // 廃城下町6層目(RUINS_FORK_FLOOR)へ進む時だけ、古城(門)方向への分かれ道を追加の選択肢として出す
  // (手描き地図の指示、2026-07-19: 廃城下町の最深部ではなく6層目の時点で分かれる)
  const offerCastleFork = !offerTeahouse && !questApproach && currentStage === "ruins" && targetFloor === RUINS_FORK_FLOOR;
  showPathChoice((pathBias) => {
    ...
    if (pathBias === CASTLE_FORK_KEY) {
      dlog("⛩️門(古城方向)へ足を踏み入れた。");
      playDungeonMoveTransition(() => moveOneFloor(CASTLE_FORK_KEY));
      return;
    }
  ...
  }, offerTeahouse, questApproach, offerCaveFork, offerValleyFork, offerCastleFork);
};
...
// 廃城下町6層目で古城(門)方向か、そのまま進むかを選ぶ分かれ道(ユーザー指示、2026-07-19)
const CASTLE_FORK_KEY = "__castle_fork__"; // 廃城下町の分かれ道で「古城方向(門)」を選んだ時専用の番人値
const RUINS_FORK_FLOOR = 6; // 廃城下町でこの階に進む時、古城(門)方向への分かれ道を追加の選択肢として提示する
// 洞窟→廃城下町→(少し森)→海の村、渓流→光る竹林の中継チェーン(2026-07-19)。各ステージの
...
const RUINSFOREST_MAX_FLOOR = 4; // 廃城下町を抜けた先の「少し森」(仮)
...
function showPathChoice(onChosen, offerTeahouse, questApproach, offerCaveFork, offerValleyFork, offerCastleFork) {
  ...
  if (questApproach) {
    ...
    // 洞窟/渓流/古城方向への分かれ道は、通常の道を選ぶという選択肢自体を必ず残す必要がある
    // (でないと分かれ道一択を強制されてしまう)。pickPathChoiceCountが1択を返した場合はここで2択に底上げする
    if ((offerCaveFork || offerValleyFork || offerCastleFork) && count < 2) count = 2;
    ...
    picked = [];
    // 茶屋/洞窟/渓流/古城方向への分かれ道は通常の抽選プールとは別枠で確定で1枠に加えるが、他の階と
    // 同じ1〜3択の分布(pickPathChoiceCount)からはみ出させない(以前は常に+1して2〜4択になり、
    // 実質「必ず3択」に偏って見えていた)。その分を差し引いた残りの枠数だけ通常の道を抽選し、
    // 特別枠は常に先頭に来るようunshiftする
    const extraSlots = (offerTeahouse ? 1 : 0) + (offerCaveFork ? 1 : 0) + (offerValleyFork ? 1 : 0) + (offerCastleFork ? 1 : 0);
    ...
    if (offerCastleFork) picked.unshift(CASTLE_FORK_KEY);
      ...
      <div class="path-choice-panel path-tags-panel${picked.length === 3 ? " path-tags-triple" : ""}${picked.length === 1 ? " path-tags-single" : ""}">
        ...
        <div class="path-choice-cards path-tags-stack${picked.length === 3 ? " path-tags-triple" : ""}${picked.length === 1 ? " path-tags-single" : ""}">
          ${picked.map((key, idx) => {
            ...
            const isCastleFork = key === CASTLE_FORK_KEY;
            const isKamikakushi = key === "kamikakushi";
            const isHikaru = key === "hikaru";
            const p = isTeahouse ? { icon: "🍡", label: "茶屋" } : isQuestApproach ? { icon: "🎯", label: "接近する" } : isCaveFork ? { icon: "🕳️", label: "洞窟" } : isValleyFork ? { icon: "🎋", label: "渓流" } : isCastleFork ? { icon: "⛩️", label: "古城方向" } : currentPathDefs()[key];
            const desc = isTeahouse ? "一休みできる茶屋が見える" : isQuestApproach ? "獲物の気配が急速に近づいてくる…" : isCaveFork ? "岩肌に空いた洞窟の入り口が見える" : isValleyFork ? "水音のする細い道が分かれている" : isCastleFork ? "門の向こうに古城の気配がする" : (flavor[key] || "");
            let extraClass = "";
            ...
            else if (isCastleFork) extraClass = " path-tag-cave";
