...
// ============ ダンジョン ============
let currentFloor = 0;
let currentStage = "forest"; // "forest"(深淵の森) | "coast"(海岸) | "cave"(洞窟) | "ruins"(廃城下町) | "gate"(門) | "castle"(古城)。
// forest/coastは町の出発ボタンで選び、enterDungeon()〜帰還/全滅まで有効。cave/ruins/gate/castleは
...
let stageEntryStack = [];
const VALID_STAGES = ["forest", "coast", "cave", "ruins", "gate", "castle"];
...
function moveOneFloor(pathBias, enterTeahouse) {
  ...
  if (retreating) {
    ...
    if (pathBias === CAVE_FORK_KEY) {
      ...
      // 中継ステージの最深部からの自動継続(洞窟→廃城下町→門→古城)。選択の余地のない一本道のため
      // showPathChoiceは経由せず、advanceBtn.onclick側からここへ直接来る
...
function currentPathDefs() {
  return currentStage === "coast" ? COAST_PATH_DEFS : currentStage === "cave" ? CAVE_PATH_DEFS
    ...
    : currentStage === "castle" ? CASTLE_PATH_DEFS : PATH_DEFS;
}
...
function currentPathFlavor() {
  return currentStage === "coast" ? COAST_PATH_FLAVOR : currentStage === "cave" ? CAVE_PATH_FLAVOR
    ...
    : currentStage === "castle" ? CASTLE_PATH_FLAVOR : PATH_FLAVOR;
}
...
// 洞窟→廃城下町→門→古城の中継チェーン(2026-07-19)。各ステージの最深部に到達すると、選択の
// 余地なく自動的に次のステージへ切り替わる(森→洞窟の分かれ道のような player choice ではない)。
// 階層数は背景画像・敵データがまだ揃っていない段階の仮決め(下地の歩行テスト用)。
// 古城はSTAGE_CHAIN_NEXTに次を持たないため、現時点で用意している一番奥として足止めされる
const RUINS_MAX_FLOOR = 12; // 廃城下町(仮)
const GATE_MAX_FLOOR = 1; // 門(仮。関門としての通過点なので1層のみ)
const CASTLE_MAX_FLOOR = 10; // 古城(仮)
const STAGE_CHAIN_NEXT = { cave: "ruins", ruins: "gate", gate: "castle" };
const STAGE_CHAIN_MAX = { cave: CAVE_MAX_FLOOR, ruins: RUINS_MAX_FLOOR, gate: GATE_MAX_FLOOR, castle: CASTLE_MAX_FLOOR };
const STAGE_CHAIN_ENTER_LOG = { cave: "🏚️廃城下町へ足を踏み入れた。", ruins: "⛩️門をくぐった。", gate: "🏯古城へ足を踏み入れた。" };
