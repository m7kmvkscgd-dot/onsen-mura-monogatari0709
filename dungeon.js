...
function moveOneFloor(pathBias, enterTeahouse) {
  ...
  if (retreating) {
    ...
    if (pathBias === CAVE_FORK_KEY) {
      ...
      // 中継ステージの最深部からの自動継続(洞窟→廃城下町→門→古城、渓流→光る竹林)。選択の余地の
...
document.getElementById("advanceBtn").onclick = () => {
  ...
  // 深淵の森5層目(VALLEY_FORK_FLOOR)へ進む時だけ、渓流への分かれ道を追加の選択肢として出す
...
// 森→渓流の分かれ道(2026-07-19、下地の歩行テスト用)。洞窟の分かれ道(10層目)と同じ階にすると
// 特別枠が2つ重なって3択になってしまい(ユーザー指示で3択は廃止済み)、通常の道を選ぶ余地が
// なくなるおそれがあるため、あえて別の階(5層目)に置いて衝突を避けている
const VALLEY_FORK_KEY = "__valley_fork__"; // 深淵の森の分かれ道で「渓流」を選んだ時専用の番人値
const VALLEY_FORK_FLOOR = 5; // 深淵の森でこの階に進む時、渓流への分かれ道を追加の選択肢として提示する
// 洞窟の最深部(2〜7層=浅い層、8〜15層=深い層、と敵データを15層までしか用意していないため、
// それより奥はまだ用意がなく進めない。この先(沼/城下町など)は未定なので、15層で足止めする
const CAVE_MAX_FLOOR = 15;
// 洞窟→廃城下町→門→古城、渓流→光る竹林の中継チェーン(2026-07-19)。各ステージの最深部に
// 到達すると、選択の余地なく自動的に次のステージへ切り替わる(森からの分かれ道のような
// player choice ではない)。階層数は敵データがまだ揃っていない段階の仮決め(下地の歩行テスト用)。
// 古城/光る竹林はSTAGE_CHAIN_NEXTに次を持たないため、現時点で用意している一番奥として足止めされる
...
function showPathChoice(onChosen, offerTeahouse, questApproach, offerCaveFork, offerValleyFork) {
  ...
  if (questApproach) {
    ...
    // 洞窟/渓流への分かれ道は、通常の道を選ぶという選択肢自体を必ず残す必要がある(でないと分かれ道
    // 一択を強制されてしまう)。pickPathChoiceCountが1択を返した場合はここで2択に底上げする
    ...
    picked = [];
    // 茶屋/洞窟/渓流への分かれ道は通常の抽選プールとは別枠で確定で1枠に加えるが、他の階と同じ1〜3択の
    // 分布(pickPathChoiceCount)からはみ出させない(以前は常に+1して2〜4択になり、実質「必ず3択」に
    // 偏って見えていた)。その分を差し引いた残りの枠数だけ通常の道を抽選し、特別枠は常に先頭に来るようunshiftする
