...
function startBattle(enemies, pathDef, encounterText) {
  peaceBattleCount++; // 戦闘後の平和な掛け合い用: 今回の探索で開始した戦闘数を数える(1戦目判定に使う)
  markEnemiesSeen(enemies); // 図鑑: 遭遇した敵を記録する(倒す必要はなく、出会った時点で登録される)
  battle = { enemies, order: [], orderIndex: 0, actingId: null, actingEnemyId: null, goldMult: (pathDef && pathDef.goldMult) || 1, justAppeared: true, omamoriUsed: {} };
...
function victory() {
  // 戦闘後の平和な掛け合い用: 1戦目が勝利で終わったかを記録する(peaceBattleCountはstartBattle()で
  // この戦闘の開始時に既にインクリメント済みなので、ここで1ならちょうど1戦目ということになる)
  if (peaceBattleCount === 1) peaceFirstBattleWon = true;
  stopBattleBgm();
  playSfx("victory");
